...
//		}
		...
		for(int i = 0; i < 1; ++i) {
			...
			int[] playerPosition = PuzzleNumber.move(arr, playerY, playerX, temp);
...
//			}
		...
		while(true) {
			...
			int[] playerPosition = PuzzleNumber.move(arr, playerY, playerX, input);
			playerX = playerPosition[0];
			playerY = playerPosition[1];
...
//			}
	...
	public static int[] move(int[][] arr, int playerY, int playerX, int temp) {
		boolean isShuffle = PuzzleNumber.shuffle(arr, playerY, playerX, temp);
		if(isShuffle) {
			switch(temp) {
			// input => 8 : 상 / 4 : 좌 / 6 : 우 / 2 : 하 
				case 2:
